...
def clean_feature(row_data):
    ...
    # 移除不参与训练的数据
    cln_data = row_data.drop(['id_card_no', 'card_name', 'loan_date', 'task'], axis=1)
    ...
    star = 'yes'
    if star == config.start_rf_select_feature:
...
# def ks_plot(mode_name, train_y_pred, y_train, val_y_pred, y_val, test_y_pred, y_test):
...
def main():
    print('正在加载数据集')
    print('--'*30)
    train_row_data = pd.read_csv(os.path.join(config.data_path, 'data_ceshi.csv'))
    # test_row_data = pd.read_csv(os.path.join(config.data_path, 'test_file.csv'))
    print('数据加载完成')

    train_row_data = train_row_data.drop_duplicates(subset=['id_card_no', 'card_name', 'loan_date', 'task'],
                                                    keep='first')
    train_row_data = train_row_data.reset_index(drop=True)

    # test_row_data = test_row_data.drop_duplicates(subset=['id_card_no', 'card_name', 'loan_date', 'task'],
    #                                               keep='first')
    # test_row_data = test_row_data[test_row_data['task'].isin([0, 1])]
    ...
    if config.button == 'on':
        ...
        # result_df = pd.DataFrame(columns=['train_ks', 'val_ks', 'test_ks'],
        #                          index=model_name_param_dict.keys())
        result_df = pd.DataFrame(columns=['train_ks', 'val_ks'],
                                 index=model_name_param_dict.keys())
        # 单个模型训练
        for mode_name, (model, param_range) in model_name_param_dict.items():
            ...
            #         val_y_pred=val_y_pred, y_val=y_val, test_y_pred=test_y_pred, y_test=y_test)
